...
export default function Landing({ username, onUsernameChange, onJoin, initialError }) {
  ...
  return (
    <div style={{ 
      ...
      >
        {theme === 'dark' ? '☀️ IGNITE LIGHT' : '🌙 EMBRACE VOID'}
      </motion.button>

      {/* Toast Notifications */}
      <div style={{ position: "absolute", top: 30, left: "50%", transform: "translateX(-50%)", zIndex: 9999, display: "flex", flexDirection: "column", gap: 12, pointerEvents: "none", width: "100%", maxWidth: 420, alignItems: "center" }}>
        <AnimatePresence>
          {toasts.map(t => (
            <motion.div
              key={t.id}
      ...
      <div style={{ flex: "1 1 50%", maxWidth: "600px", zIndex: 1, paddingRight: "40px" }}>
